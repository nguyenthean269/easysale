      ...
      <div class="bg-white p-6 rounded-lg shadow-sm">
        <div class="space-y-6">
          <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-800">Căn hộ chung cư bán</h1>
          </div>
            ...
            <nz-table 
              ...
              <tbody>
                <tr *ngFor="let apartment of apartmentsTable.data">
                  ...
                  <td>
                    <div *ngIf="apartment.price" class="font-medium text-blue-600">
                      {{ formatPrice(apartment.price) }}
                    </div>
                    <div *ngIf="apartment.price_early" class="text-sm text-gray-500">
                      Sớm: {{ formatPrice(apartment.price_early) }}
                    </div>
                  </td>
                  <td>
...
export class CanHoChungCuBanComponent implements OnInit, OnDestroy {
  ...
  parsePathParams() {
    ...
    if (typeof window !== 'undefined') {
      ...
      // URL format: /can-ho-chung-cu-ban,du-an-32,dien-tich-tu-20m,dien-tich-toi-50m
      
      // Tìm phần chứa 'can-ho-chung-cu-ban'
      const pathSegment = url.split('/').find(part => part.includes('can-ho-chung-cu-ban'));
  ...
  applyFilters() {
    // Tạo path với format: can-ho-chung-cu-ban,du-an-32,dien-tich-tu-20m,dien-tich-toi-50m
    const pathParts: string[] = ['can-ho-chung-cu-ban'];
  ...
  resetFilters() {
    ...
    this.pageIndex = 1;
    this.router.navigateByUrl('/can-ho-chung-cu-ban', { replaceUrl: true });
  }
  ...
  getStatusColor(status: string): string {
    ...
    if (statusUpper.includes('AVAILABLE') || statusUpper.includes('SẴN')) {
      return 'green';
    } else if (statusUpper.includes('SOLD') || statusUpper.includes('ĐÃ BÁN')) {
      return 'red';
