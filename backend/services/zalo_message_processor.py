...
class ZaloMessageProcessor:
    ...
    def __init__(self):
        ...
        self.thread = None
        self.interval = 600  # 10 phút = 600 giây
        ...
        logger.info("ZaloMessageProcessor initialized")
    
    def get_zalo_db_connection(self):
        """Tạo kết nối database zalo_messages"""
        try:
            logger.info("Attempting to connect to zalo_messages database...")
            logger.info(f"Database URI: {zalo_app.config.get('SQLALCHEMY_DATABASE_URI')}")
            ...
            logger.error(f"❌ Zalo database connection error: {e}")
    ...
    def get_warehouse_db_connection(self):
        ...
        try:
            logger.info("Attempting to connect to warehouse database...")
            logger.info(f"Database URI: {warehouse_app.config.get('SQLALCHEMY_DATABASE_URI')}")
            ...
            logger.error(f"❌ Warehouse database connection error: {e}")
            logger.error(f"Error type: {type(e)}")
            return None
...
{{
  ...
  "additionalProperties": false
    ...
    def map_unit_type_to_id(self, unit_type_name: str) -> Optional[int]:
        """
        Map unit type name sang ID
        
        Args:
            unit_type_name: Tên loại căn hộ
        ...
        if unit_type_name in self.unit_type_mapping:
            return self.unit_type_mapping[unit_type_name]
        
        # Tìm partial match
        for name, id_val in self.unit_type_mapping.items():
            if unit_type_name.lower() in name.lower() or name.lower() in unit_type_name.lower():
    ...
    def insert_or_update_apartment(self, apartment_data: Dict) -> bool:
        """
    ...
    def run_scheduler_mode(self):
        ...
        logger.info(f"⏰ Running in SCHEDULER mode (interval: {self.interval}s)")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"Processing completed in {elapsed_time:.2f}s. Sleeping for {sleep_time:.2f}s")
    ...
    def start(self):
        ...
        logger.info("ZaloMessageProcessor service started")
